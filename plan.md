...
**Status: Phase 3.1, 3.2, 3.3.1, 3.3.2a, 3.3.2b, 3.3.3, 3.3.4a, 3.3.4b, 3.3.5, 3.3.6a, 3.3.6b, 3.4.1, 3.4.2, and 3.4.3a complete; Phase 3 not passed.**
...
- **3.3.6b complete:** authenticated `POST /api/v1/jobs/{jobId}/cancel` and bounded authenticated `GET /api/v1/jobs/{jobId}/events` are implemented over separate read/cancellation capabilities. Cancellation applies type-aware RBAC, validated durable read-before-denial, live origin/CSRF checks, safe 202 `JobResponse` replay, fixed error mapping, and `jobs.cancel` generic audit hooks. SSE emits only compact redacted `JobResponse` projections, hashes exact event bytes, suppresses unchanged states, closes at terminal state, and is bounded to 250ms polling, 20 seconds, 64 events, 32 concurrent streams per handler, and 4096 serialized data bytes without cancel-on-disconnect. Strict q parsing, no hop-by-hop Connection header, 429 Retry-After behavior, OpenAPI, route contract tests, full role/status/error/framing/audit matrices, and production scheduler cancellation integration are covered. Verification timestamp: 2026-09-03T11:44:39Z.
- **3.4.1 complete:** `internal/state` now provides `NewAuditEvent` as a narrow caller-ID projection boundary from `audit.Event`, normalizing actors through `ActorFromAuth`, dropping display/session/CSRF/token-bearing fields, canonicalizing time to UTC, accepting bounded future namespaced actions, enforcing current durable job IDs for newly projected events, and mapping all constructor validation failures to the safe `ErrInvalidAuditEvent` sentinel. Canonical encoding omits empty optionals; the strict typed decoder preserves v1 read compatibility for explicit-null optionals and legacy safe-code job references while rejecting duplicate/unknown/non-canonical/trailing/wrong-header input within the common document bound. Focused tests cover the actor/outcome/action/role/method/identifier matrix, exact bytes, v1 compatibility, strict decoding, round-trip, and sentinel scans. No persistence, redaction, rotation, recovery, HTTP, or state layout/version change was added. Verification timestamp: 2026-09-03T12:58:29Z.
- **3.4.2 complete:** `internal/auditenvelope` now provides an immutable pre-storage boundary containing only unexported canonical `AuditEvent` bytes. `New` calls `state.NewAuditEvent` then `state.EncodeAuditEvent`, copies the bytes, and maps failures to fixed `ErrInvalidEnvelope`; `Bytes` returns a defensive copy; `Validate` rejects zero, malformed, non-canonical, or over-bound values using strict decode plus canonical re-encode equality. Focused tests cover exact full/minimal bytes and state validation, structural safe metadata allowlisting, excluded transient fields, defensive-copy behavior, all current/future actions and outcomes, anonymous denied/failed events, unsafe input and sentinel safety, grammar-valid producer-owned reason text, and the inherited document bound. The package imports only `internal/audit`, `internal/state`, and standard library packages. No schema, ID generation, persistence, statefs, sink, recorder, runtime, HTTP, or call-site behavior changed. Verification timestamp: 2026-09-04T10:54:12Z.
- **3.4.3a complete:** `internal/auditsegment` provides pure immutable bounded JSON segments around canonical audit envelopes. It uses the versioned compact wrapper `apiVersion`, `sequence`, `recordCount`, and `records`, record-level lowercase SHA-256 corruption checksums over exact canonical event bytes, a 1,024-record limit, and a 4 MiB complete-document limit. Empty records are `[]`; strict wrapper duplicate/unknown/casing/trailing/non-canonical checks, state event decode/re-encode, duplicate-ID rejection, sequence binding, bounded hostile-array handling, ordered immutable record lookup, and fixed safe sentinels are covered. The package is filesystem/context/runtime/storage-free. Verification timestamp: `2026-09-04T12:33:07Z`.
- **Next:** 3.4.3b — persist and rotate audit segments atomically; durable runtime audit call-site coverage remains future work. Diffing and saved plans remain future work.
