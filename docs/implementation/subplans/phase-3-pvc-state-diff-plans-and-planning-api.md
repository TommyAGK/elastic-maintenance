...
- **Next:** 3.4.3b — persist and rotate audit segments atomically.

#### 3.4.3a Define the bounded deterministic JSON audit segment codec — **Complete**

- **Scope:** Define a versioned deterministic JSON segment codec around canonical safe-envelope payloads. The compact wrapper has the fixed fields `apiVersion`, `sequence`, `recordCount`, and `records`; each record has the fixed fields `sha256` and `event`, with SHA-256 covering the exact canonical `AuditEvent` bytes. Segment files are JSON storage artifacts, not a new `state` document kind.
- **Dependencies:** 3.4.2 and the strict state codec.
- **Definition of done:** New/append/decode operations preserve exact canonical envelope bytes, reject zero or mismatched sequences, duplicate event IDs, wrapper unknown/duplicate/case-variant fields, malformed/trailing/non-canonical JSON, checksum-invalid records, count/size violations, and impossible append bounds using fixed safe errors. Empty records encode `[]`; the codec is filesystem- and context-free.
- **Focused verification:** Cover exact golden JSON, empty/one/many records, deterministic encoding, envelope preservation, every wrapper/event corruption form, checksum mismatch, truncation at every byte boundary, duplicate IDs, count and 4 MiB limits, hostile arrays within the input cap, non-canonical payloads and explicit nulls, legacy dotted job compatibility, append-full behavior, and input/output defensive copies.
- **Worker boundary:** A segment-codec worker owns only the pure JSON codec and format documentation; it does not own statefs, filenames, repository concurrency, rotation, recovery policy, ID generation, runtime, or HTTP.
- **Evidence:** `internal/auditsegment` provides immutable `Segment` values with `New`, strict sequence-bound `Decode`, typed envelope-only `Append`, defensive `Bytes`, `Sequence`/`Count` metadata, and ordered immutable `Records` for repository replay lookup. The exact versioned wrapper, fixed key order, `[]` empty-array form, 4 MiB complete-document limit, 1,024-record limit, lowercase SHA-256 corruption checksums, strict state decode/canonical re-encode, duplicate-key scanning, duplicate-ID detection, bounded records decoding, and fixed sentinel taxonomy are covered by focused tests. The package imports only `internal/auditenvelope`, `internal/state`, and the standard library; it has no statefs, storage, runtime, or HTTP dependency.
- **Verification:** Focused segment tests, full tests, race tests, vet, build/cross-build, contract fixtures, and `git diff --check` pass. Verification timestamp: `2026-09-04T12:33:07Z`.
- **Next:** 3.4.3b — persist and rotate audit segments atomically.

#### 3.4.3b Persist and rotate audit segments atomically

- **Scope:** Implement the bounded audit segment repository under fixed `statefs.AuditDir`. Its append API accepts only a safe envelope, never `audit.Event`, `state.AuditEvent`, or arbitrary bytes. Rewrite the active bounded JSON segment atomically with existing statefs fsync/rename primitives; rotate to deterministic sequence filenames before an append would exceed the JSON segment limits.
...
- **Worker boundary:** A segment-storage worker owns repository layout/rotation and any narrowly required statefs primitive; it does not own codec details, redaction, ID generation, runtime startup, recorder integration, corruption recovery, authorization, or HTTP reads.

#### 3.4.4 Recover audit segments after restart and partial writes

- **Scope:** Add bounded segment recovery that validates complete JSON segment documents and every event with the strict state decoder, preserves valid prior events, and resumes append safely after clean or interrupted shutdown. Only an incomplete trailing JSON document may be handled according to the documented contract; non-trailing corruption and unsafe structure fail closed.
- **Dependencies:** 3.4.3b.
...
- **Focused verification:** Cover truncated final JSON documents, interrupted rotation, invalid segment JSON, missing/duplicate segments, non-trailing corruption, oversized scans, clean restart, reruns, ordering, no invented events, and secret-free diagnostics.
