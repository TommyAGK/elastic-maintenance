...
Substep 2.1 is implemented. Browser authentication now uses lazy, explicitly origin-pinned OIDC discovery, Authorization Code with PKCE/state/nonce, strict callback-origin and token validation, bounded redirect-free provider/JWKS HTTP, configured claim-to-role mapping, and stateless protected sessions. Session and transaction cookies are purpose-separated AES-GCM envelopes backed by a live mounted key ring; current plus at most two previous keys provide bounded rotation overlap. Existing sessions authenticate without contacting the IdP, while login/callback fail closed during provider outages. Linux reads projected Secret keys descriptor-relatively with `openat2` beneath the configured mount; the non-Linux test fallback requires an immutable administrator-controlled root. Substep 2.2 is also implemented: explicit audited emergency login, pinned Argon2id verification, bounded throttling and lockout, fixed 15-minute administrator sessions, live credential-set/session-key revocation, IdP-outage operation, UI warnings, and the mandatory provisioning/rotation runbook are complete. Substep 2.3 is implemented as well: external API clients can present bounded OIDC JWT bearer tokens validated against the same pinned issuer, audience, algorithms, JWKS transport, claim mapping, and RBAC actor model. Cookie-plus-bearer and duplicate authorization identities fail closed. Substep 2.4 is implemented: the centralized permission matrix is exercised across every protected endpoint and role, unknown/no-role actors are denied, OIDC and break-glass login issuance is audit-gated, and authenticated logout plus mutation attempts emit safe actor/action/outcome hooks with target or plan identifiers where applicable. Production currently sends these hooks to structured security logs; Phase 3 will add durable JSON/PVC audit persistence. Substep 2.5 is implemented as a narrow namespaced Kubernetes Secret boundary with in-cluster service-account construction, injectable tests, strict namespace/name-prefix checks, state/target ownership metadata, optimistic update/delete preconditions, safe error classification, and RBAC that grants only get/create/update/delete without list/watch. Substep 2.6 is implemented: administrator-only credential status/upload/rotation/deletion APIs enforce strict JSON, idempotency, browser Origin plus CSRF validation, bearer automation separation, printable bounded API keys, canonical CA-only PEM bundles, atomic usage/deletion coordination, owned-Secret metadata, safe status-only responses, and upload-versus-rotation audit actions. Substep 2.7 is implemented: internal target-client leases revalidate exact owned Secret material, bind it to live target identity, block deletion while leased, combine system and uploaded CA roots without mTLS, disable proxying and redirects, bound transport lifetime and responses, send API keys only as Kibana authorization headers, and clear/detach retained buffers and roots on close. Substep 2.8 is implemented: the Kibana HTTP core builds base-path and space-aware URLs, probes and enforces stable `>=9.2.0,<10.0.0` versions, bounds requests/responses/headers/time, rejects redirects and malformed protocols, classifies remote failures without body disclosure, retries only narrow transient idempotent reads, and exhausts cursor, Fleet page/perPage, and rule page/per_page pagination with fail-closed totals/cursor invariants across both fixture versions. Substep 2.9 is implemented: typed adapters read installed and exact integration packages, caller-ID agent/package policies, caller-`rule_id` custom rules, and collective prebuilt status across both fixture versions. They validate strict wire Unicode and required fields, preserve defensively cloned complete-update baselines, retain unsafe rule collisions without treating them as manageable, produce versioned RFC-canonical live fingerprints from managed projections, and exclude generated revision/timestamp/ownership drift. Substep 2.10 is implemented: viewer-authorized readiness and compatible-version probes plus CSRF/origin-protected asynchronous live-inventory jobs use full captured target identity, leased credentials, bounded process-local queues/results, stable typed resource ordering, safe failure codes, and paginated no-baseline responses. The embedded UI exposes readiness, live inventory polling/pagination, viewer credential status, and administrator-only upload/rotation/deletion without browser storage or readback. Substep 2.11 and the Phase 2 gate are complete: authentication, break-glass, RBAC, CSRF/origin, Secret ownership, credential validation/redaction, TLS/CA/spaces/pagination/error/timeout contracts, canonical fixture adapters, identity-change rejection, lease shutdown, replay safety, OpenAPI parity, UI syntax, race tests, cross-compilation, builds, fixture verification, and smoke checks pass.
