...
func TestEmbeddedOperatorInterfaceIsSelfContainedAndReadOnly(t *testing.T) {
	...
	for _, required := range [][]byte{[]byte("/api/v1/session"), []byte("/api/v1/sources"), []byte("/api/v1/targets"), []byte("/api/v1/validations"), []byte("method:\"POST\""), []byte("Idempotency-Key"), []byte("/auth/logout"), []byte("authenticationMethod"), []byte("/auth/break-glass/login"), []byte("password.value=\"\""), []byte("/credential-status"), []byte("/credentials"), []byte("/inventory"), []byte("X-CSRF-Token"), []byte("apiKey.value=\"\"")} {
