...
async function openTarget(id){
  try{const body=await api(`/api/v1/targets/${encodeURIComponent(id)}?pageSize=100`),item=body.target,panel=$("#target-detail");const labels=(item.labels||[]).map((label)=>`${label.key}=${label.value}`).join(", ")||"None";
    panel.replaceChildren(detailHeader("Kibana target",item.identity.name,()=>panel.classList.add("hidden")),definitions([["Source set",item.resourceSetID],["External revision",item.revision||"Not supplied"],["Kibana URL",item.identity.url],["Space",item.identity.space],["Labels",labels],["Desired digest",digest(item.desiredDigest)]]),node("p",{class:"readonly",text:"Target assignment and desired resources are read-only. Update mounted server configuration through the external deployment workflow."}),node("h3",{text:"Applicable resources"}),resourceList(item.resources));
    if(body.nextResourcePageToken)panel.append(node("p",{class:"location",text:"Additional resource metadata is available through the paginated /api/v1 endpoint."}));panel.classList.remove("hidden");panel.scrollIntoView({behavior:"smooth",block:"start"});
  }catch(error){showError(error);}
}
...
async function startValidation(){
  ...
  try{const key=`web-${Date.now()}-${crypto.randomUUID()}`;const body=await api("/api/v1/validations",{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":key},body:"{}"});toast("Validation queued.");await loadValidations();activateView("validations");await openValidation(body.job.id);}
